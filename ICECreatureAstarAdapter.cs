...
namespace ICE.Creatures.Adapter
{
	 ...
	 */
		...
		{
			if( Time.time - m_LastRepath >= RepathRate && m_CanSearchAgain && CanSearch ) 
			{
				SearchPath ();
				return RepathRate;
			} 
			else 
			{
				...
				float v = RepathRate - (Time.time-m_LastRepath);
				return v < 0 ? 0 : v;
			}
		}
		...
		public virtual Vector3 GetFeetPosition() 
		{
			if( m_CharacterController != null ) 
				return m_Transform.position - Vector3.up * m_CharacterController.height * 0.5F;
			else
	 ...
	 * Filled in by #CalculateVelocity */
		...
		protected Vector3 CalculateVelocity( Vector3 _position ) 
		{
			...
			float _forward_speed = m_Controller.Creature.Move.MoveVelocity.z;
		...
		protected virtual void RotateTowards( Vector3 _direction ) {
			...
			float _turning_speed = m_Controller.Creature.Move.MoveAngularVelocity;
